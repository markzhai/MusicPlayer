...
public class MusicPlayer extends PApplet {
	...
	/*
	 * public void setup() { size(300, 300); background(255); fill(0);
	 * 
	 * // Instantiate the UI - Using "this" to make it possible for the UI to //
	 * reference this SwtUI mySwtUI = new SwtUI(this); Thread mySwtUIThread =
	 * new Thread(mySwtUI);
	 * 
	 * // Start the UI-threads mySwtUIThread.start();
	 * 
	 * }
	 * 
	 * // position is public, to make it accessible from SwtUI public int
	 * position = 30;
	 * 
	 * public void draw() { background(255); ellipse(position, 150, 25, 25); }
	 */
	...
	public void setup() {
		...
		player = minim.loadFile("chata_xi_part1.mp3");
		player.play();
		...
		playlist_skin = loadImage("playlist_skin.bmp");
		seeker = loadImage("progress_thumb.bmp");

		// font = createFont("SimHei", 20);
		font = loadFont("SimHei-20.vlw");

		btnEqualizer = new GImageToggleButton(this, 403, 23, "equalizer.bmp",
				"equalizer_2.bmp", 2, 1);
	...
	public void draw() {
		...
		image(playlist_skin, 0, 77);
		// background(240);
		// 184, 55 ~ 300

		image(seeker,
				184 + (int) (player.position() / duration * (116 - seeker.width)),
				55);
		...
		text(meta.author() + " - " + meta.title(),
				(player_skin.width - textWidth(meta.title())) / 2 - 45, 48);

		int timeLeft = player.position() - player.length();
		String timeLeftStr = String.format("%02d:%02d", timeLeft / 1000 / 60,
				-timeLeft / 1000 % 60);
		text(timeLeftStr,
				(player_skin.width - textWidth(timeLeftStr)) / 2 + 92, 63);
